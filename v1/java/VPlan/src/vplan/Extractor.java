...
public class Extractor {
    ...
    private void createElements(String str) {
        ...
        String lehrer = "";
        String sonstiges = "";
        for (int z = 0; z < lines.size() / 10; z++) {
            int i = 10 * z;
            String zeile = lines.get(i);
            zeile = lines.get(i + 1);
            klasse = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 2);
            std = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 3);
            vertretung = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 4);
            fach = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 5);
            raum = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 6);
            statt = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 7);
            lehrer = getPartOfString(zeile, "Vertretungen\">", "</td>");
            zeile = lines.get(i + 8);
            sonstiges = getPartOfString(zeile, "Vertretungen\">", "</td>");
    ...
    private void extractTitel(String str) {    
        tag = getPartOfString(str, "<Title>", "<");
        tag = removeWeirdness(tag, "ü");
        String s = "Ehrenbürg-Gymnasium Forchheim Vertretungsplan für ";
        int len = s.length();
        if (tag.startsWith(s)) {
    ...
    private static String getPartOfString(String s, String s1, String s2) {
        String erg = "";
        int m = s.indexOf(s1);
        if (m > -1) {
